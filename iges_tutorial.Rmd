...
mod1 <- lm(sim_height ~ mod_sig + sex + sim_age + PC1 + PC2 + PC3 + PC4 + PC5, data = res)
mod2 <- lm(sim_height ~ mod_sug + sex + sim_age + PC1 + PC2 + PC3 + PC4 + PC5, data = res)
mod3 <- lm(sim_height ~ mod_inf + sex + sim_age + PC1 + PC2 + PC3 + PC4 + PC5, data = res)

# Get predicted heights from models
pred1 <- predict(mod1, res)
pred2 <- predict(mod2, res)
pred3 <- predict(mod3, res)


# Define p-value thresholds and calculate r^2s
mods <- c(5e-8, 1e-5, 1)

perfs <- c(cor(pred1, res$sim_height)^2, 
           cor(pred2, res$sim_height)^2, 
           cor(pred3, res$sim_height)^2)
...
ggplot(df, aes(as.factor(mods), perfs)) +
  ...
  labs(y = expression(R^2), x = "p threshold")
```
