...
rownames(scores) <- pooled$fam$sample.ID
names(scores) <- c("mod_sig", "mod_sug", "mod_inf")

# Merge with sample information
res <- merge(pd, scores, by.x = "IID", by.y = "row.names")

# Save the scores
write.csv(res, "scores.csv")
```
...
#### Goal: 
Use the PRS weights derived in Part I to calculate scores for individuals in a 1000 Genomes sample. Then evaluate and compare the performance of multiple scores across diverse ancestry groups.
...
2. PRS weights from session 1

3. (Optional) Terminal access for running ESCALATOR pipeline

#### Description of provided files:
* genetic file names (only needed for optional PRS calculation) = 1000_genomes.9.ALL.mafmiss.*
    * 1000_genomes.9.ALL.mafmiss.psam = sample information for unrelated individuals filtered for missingness (<0.05)
    * 1000_genomes.9.ALL.mafmiss.pvar = variant information for SNPs filtered for minor allele frequency (MAF > 0.01) and missingness (<0.05)
    * 1000_genomes.9.ALL.mafmiss.pgen = [binary file format with complex genetic infomation](https://www.cog-genomics.org/plink/2.0/input#pgen) 
...
In this section, we will use the containerized [ESCALATOR pipeline](https://github.com/menglin44/escalator) for score harmonization and calculation. More details and examples can be found in the GitHub repository. 

### Download PRS Weights

Use terminal commands to download an example weight file from the [PGS Catalog](https://www.pgscatalog.org/):
...
Within the main working folder, make a sub-directory to host score outputs:
...
Now that the PRS weights and calculated scores are ready, we will evaluate them.
